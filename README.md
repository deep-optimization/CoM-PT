...
*We recommand using ```numpy<2.0``` in this repository to avoiod unnecessary issues in training.*
...
*We also provide an URL that you can directly download .zip file.* 
...
*As CC12M dataset is extremely large, the zip file is still in preparation for release.* 

### Image Descriptions of CC3M and Merged-15M

We do not directly use the generated `cc3m_train.csv` and `cc12m_train.csv`  file in our training. Instead, we combine them with MLLM generated long captions in DreamLIP. You can download the `cc3m_lc.csv` and `cc12m_lc.csv`  in this URL.



## Model Training
Training scripts are all provided in  `training_script` folder. Note that the path of teacher's checkpoint should be correctly modified before conducting CoM-PT.

Then you can conduct baseline pre-traning as:
````
