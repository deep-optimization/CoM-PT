...
class ClipLoss(nn.Module):
    ...
    def forward(self, image_features, text_features, logit_scale):
        ...
        #     flag=True
        #     num_text_per_img = int(text_features.shape[0] / image_features.shape[0])
        #     num_pair = image_features.shape[0] * self.world_size
        if self.world_size > 1:
            ...
            if self.local_loss:
                ...
                # if flag:
                #     logits_per_image = logits_per_image.view(num_pair, num_pair, num_text_per_img).sum(dim=2)
                #     logits_per_text = logits_per_text.view(num_pair, num_text_per_img, num_pair).sum(dim=1)
            else:
        ...
        if image_features.shape[0] != text_features.shape[0]:
            ############## Multi-positive loss ####################################
            number_text = int(text_features.shape[0] / image_features.shape[0])
            # labels_mul = self.get_ground_truth_multicap(device, 
            #                     all_image_features.shape[0], number_text, 1)
            # total_loss = 0
            # total_loss += self.pairwise_circleloss(logits_per_image, targets) 
            ...
            for i in range(number_text):
                logits_per_image_raw = logits_per_image.view(all_image_features.shape[0], all_image_features.shape[0], number_text)[:,:,i]
                logits_per_text_raw = logits_per_image_raw.T

                total_loss += (
                    F.cross_entropy(logits_per_image_raw, labels) +
                    F.cross_entropy(logits_per_text_raw, labels)
                    ) / 2
            total_loss /= number_text
            ########################################################################
            # print(total_loss)
            # assert 0 == 1
...
class KDClipLoss(nn.Module):
    ...
    def forward(self, image_features, text_features, logit_scale, \
        t_image_features, t_text_features, t_logit_scale, epoch=0):
        device = image_features.device
        flag = False
        if image_features.shape[0] != text_features.shape[0]:
            flag=True
            num_text_per_img = text_features.shape[0] / image_features.shape[0]
            num_pair = image_features.shape[0]
        ...
        if self.args.use_longcap:
            ...
            task_loss  = total_loss / number_text
        ...
        hinge_fd_loss_image = F.mse_loss(normalized_all_image_features, t_all_image_features, reduction='none').mean(dim=1)
        hinge_fd_loss_text  = F.mse_loss(normalized_all_text_features, t_all_text_features, reduction='none').mean(dim=1)
        hinge_fd_loss = hinge_fd_loss_image.mean() + hinge_fd_loss_text.mean()

            
        fd_loss = self.args.alpha_fd_loss * hinge_fd_loss
        
        return task_loss, fd_loss
...
#         # Define Hinge FD Loss
#         student_logits = F.softmax(logits_per_image, dim=1)
#         teacher_logits = F.softmax(t_logits_per_image, dim=1)
#         logits_distance = torch.abs(student_logits - teacher_logits).sum(dim=1)

#         # if epoch < 5:
#         #     ratio = 1.0
#         # else:
#         #     ratio = 0.9**(epoch-5+1) * 0.5 + 0.5

#         # ratio = 0.9**(epoch-1) * 0.5 + 0.5

#         # ratio = 0.9**(epoch+1)



#         ### Drop easyest samples #################

#         # ratio = 0.7 # Drop 30% of the easyest samples
#         # if epoch < 5:
#         #     ratio = 1.0
#         # else:
#         #     ratio = 0.9**(epoch-5) * 0.5 + 0.5

#         ratio = 1
#         # epoch = max(epoch, 0)
#         # ratio = 0.9**(epoch) * 0.3 + 0.7

#         # ratio = 1

#         length = logits_distance.shape[0]

#         preserve_length = ratio * length
#         # mask_length = ratio * length

#         top_k, top_k_indice = torch.topk(logits_distance, int(preserve_length))
...
#          ### Drop hardest samples ################

#         # ratio = 0.3 # Drop 30% of the hardest samples
...
#         hinge_fd_loss_image = F.mse_loss(normalized_all_image_features, t_all_image_features, reduction='none').mean(dim=1)
#         hinge_fd_loss_text  = F.mse_loss(normalized_all_text_features, t_all_text_features, reduction='none').mean(dim=1)
#         hinge_fd_loss = torch.mul(hinge_fd_loss_image, mask).mean() + torch.mul(hinge_fd_loss_text, mask).mean()

#         # hinge_fd_loss = hinge_fd_loss /(1-ratio)

#         hinge_fd_loss = hinge_fd_loss / ratio
