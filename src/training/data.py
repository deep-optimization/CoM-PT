...
import nltk
import random
nltk.download('punkt_tab')  # 下载分词器模型
...
class CsvDataset(Dataset):
    def __init__(self, data_root, input_filename, transforms, img_key, caption_key, use_longcap=False, sep="\t", merged_num=1, num_text=1, tokenizer=None):
    ...
    def split_caption(self, text):
        subcap = split_paragraph_into_sentences(text)
    ...
    #     text_list = split_paragraph_into_sentences(text)
    ...
    #         processed_text = text_list[0] + ' ' + text_list[index]
    ...
    def __getitem__(self, idx):
        if self.use_longcap:
            ...
            # texts = self.tokenize([self.process_text(idx)])
            return images, texts
...
# use_longcap=False, sep="\t", merged_num=1, num_text=1
...
def get_data(args, preprocess_fns, epoch=0, tokenizer=None):
    preprocess_train, preprocess_val = preprocess_fns
    data = {}
    # print(args.use_longcap)
    # assert 0 == 1
    flag = args.use_longcap
    print(flag)
    ...
    args.use_longcap = False # The validation set does not use long caption file
