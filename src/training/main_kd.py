...
def weight_initialization(ckpt_t, ckpt_s, differ_width=False): 
    # As the teacher in CoM is smaller than the student, the teacher's key also appears in the student. 
    for key in ckpt_t.keys(): 
        if key.find('module.visual.transformer') !=-1 and differ_width is True: 
            # This process is activated when the width is different 
            key_list = key.split('.') 
            layer = int(key_list[4])
            key_list_1 = key_list.copy() 
            key_list_2 = key_list.copy() 
            key_list_1[4] = str(2*layer) 
            key_list_2[4] = str(2*layer+1) 
            key_1 = '.'.join(key_list_1) 
            key_2 = '.'.join(key_list_2) 
            value_s_list = [(key_1, ckpt_s[key_1]), (key_2, ckpt_s[key_2])] 
            value_t = ckpt_t[key] 
            for key, value_s in value_s_list: 
                if len(value_t.shape) == 0: 
                    value_s = value_t 
                elif len(value_t.shape) == 1: 
                    value_s[:value_t.shape[0]] = value_t 
                elif len(value_t.shape) == 2: 
                    value_s[:value_t.shape[0], :value_t.shape[1]] = value_t 
                elif len(value_t.shape) == 3: 
                    continue 
                elif len(value_t.shape) == 4: 
                    value_s[:value_t.shape[0],:,:,:] = value_t 
                else: 
                    raise Exception() 
                ckpt_s[key] = value_s 
...
def main(args):
    ...
    logging.info('The teacher model is loaded successfully.')

    if args.weight_init:
        student_dict = model.state_dict()
        if args.differ_width:
            logging.info("The student model has different width from the teacher model.")
        else:
            logging.info("The student model has the same width as the teacher model.")

        student_dict = weight_initialization(checkpoint, student_dict, differ_width=args.differ_width)
        model.load_state_dict(student_dict)
        logging.info("The student model is initilized by the teacher model.")
    else:
        logging.info("The student model is training-from-scratch.")
